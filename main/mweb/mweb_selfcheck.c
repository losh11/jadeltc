...
/* ── Kernel signing vector ───────────────────────────────────────────── */

static bool test_mweb_kernel_sign(void)
{
    /* Pinned e_k from the SIMPLE signing vector */
    ...
    /* Expected E_k from the SIMPLE vector */
    ...
    /* Signature is deterministic for pinned e_k — checked exhaustively
     * in native tests; selfcheck verifies the excess to confirm the
     * kernel-signing code path is reachable and structurally sound. */
 ...
 *
 * Two fixtures from the native-host test set (test_mweb_output.c):
 *   1. HAPPY          — full-field derivation check
 *   2. INVALID_SCALAR — rejects sender_key == 0 with the right error code
 *
 * Fixture values are pinned copies of OUT_HAPPY_* from
 * tests/fixtures/output_vectors.h.
 */
static bool test_mweb_derive_output(void)
{
    /* Inputs: sender_key = 0x11..11, stealth address derived from
     *   scan_sk = 0x01..01 → A,  spend_sk = 0x02..02 → B. */
    static const uint8_t SENDER_KEY[32] = {
    ...
    static const uint8_t SCAN_A[33] = {
    ...
    static const uint8_t SPEND_B[33] = {
    ...
    static const uint64_t VALUE = 100000000ULL;

    static const uint8_t EXPECTED_BLIND[32] = {
    ...
    static const uint8_t EXPECTED_KO[33] = {
    ...
    static const uint8_t EXPECTED_KS[33] = {
    ...
    static const uint8_t EXPECTED_KE[33] = {
    ...
    static const uint8_t EXPECTED_COMMIT[33] = {
    ...
    static const uint8_t EXPECTED_VIEW_TAG = 0x1f;
    static const uint64_t EXPECTED_MASKED_VALUE = 0x990f3e2289a63b46ULL;
    static const uint8_t EXPECTED_MASKED_NONCE[16] = {
        0x75,0x0d,0x01,0xb1,0x7b,0x1c,0x7b,0x61,
        0x74,0x78,0x60,0xf4,0x30,0x0f,0x0b,0xf7,
    };

    /* Happy path — all eight fields must match byte-for-byte. */
    struct mweb_derived_output got;
    if (mweb_derive_output(SENDER_KEY, SCAN_A, SPEND_B, VALUE, &got)
        != MWEB_OK) { FAIL(); }
    if (memcmp(got.blind,               EXPECTED_BLIND,  32) != 0) { FAIL(); }
    if (memcmp(got.output_pubkey,       EXPECTED_KO,     33) != 0) { FAIL(); }
    if (memcmp(got.sender_pubkey,       EXPECTED_KS,     33) != 0) { FAIL(); }
    if (memcmp(got.key_exchange_pubkey, EXPECTED_KE,     33) != 0) { FAIL(); }
    if (memcmp(got.commit,              EXPECTED_COMMIT, 33) != 0) { FAIL(); }
    if (got.view_tag                 != EXPECTED_VIEW_TAG)         { FAIL(); }
    if (got.masked_value             != EXPECTED_MASKED_VALUE)     { FAIL(); }
    if (memcmp(got.masked_nonce,       EXPECTED_MASKED_NONCE, 16) != 0) { FAIL(); }

    /* Error path — sender_key == 0 must be rejected before any EC op. */
    static const uint8_t ZERO[32] = {0};
    if (mweb_derive_output(ZERO, SCAN_A, SPEND_B, VALUE, &got)
...
bool test_mweb_crypto(void)
{
    ...
    if (!test_mweb_kernel_sign()) { return false; }
    if (!test_mweb_derive_output()) { return false; }
    return true;
}
